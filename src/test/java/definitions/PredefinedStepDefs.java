...
public class PredefinedStepDefs {
    ...
    public void iCopyTextFromXpathAndPastItIn(String input, String output) {
        String el1 = getDriver().findElement(By.xpath("(//span[@unselectable='on'])[1]")).getText();
        String el2 = getDriver().findElement(By.xpath("(//span[@unselectable='on'])[2]")).getText();
        String el3 = getDriver().findElement(By.xpath("(//span[@unselectable='on'])[3]")).getText();
        String text = el1 + el2 + " " + el3;

        String[] fullText = text.split("");
        for (String letter : fullText) {
            getDriver().findElement(By.xpath("//input[@type='text'][@class='txtInput']")).sendKeys(letter);
        }
    }
}